// Camera La/Lb Measurement Script
let serialPort = null;
let camera = null;
...
async function connectPrinter() {
    ...
    try {
        ...
        safeUpdateElement(connectBtn, 'textContent', 'Connected');
        if (connectBtn) connectBtn.disabled = true;
        
        addLogEntry('Printer connected successfully', 'info');
        
        // Enable measurement buttons if camera is also ready
...
function handleSerialDisconnect(event) {
    ...
    safeUpdateElement(document.getElementById('connectBtn'), 'textContent', 'Connect Printer');
...
async function startCamera() {
    ...
    try {
        ...
        safeUpdateElement(startBtn, 'textContent', 'Camera Active');
        if (startBtn) startBtn.disabled = true;
        
        addLogEntry('Camera started successfully', 'info');
        
        // Enable measurement buttons if printer is also connected
        ...
        addLogEntry('Camera failed: ' + error.message, 'error');
    }
}
...
function enableMeasurementButtons() {
    ...
    document.getElementById('statusMessage').textContent = 
        'Ready to measure la/lb parameters. Use crosshair to align nozzle.';
}
...
function nextMeasurementStep() {
    ...
    if (measurementState.currentPhase === 'lb') {
        document.getElementById('measurementInstruction').innerHTML = 
            `<p>Use movement controls to position the nozzle at the center of the camera crosshair, then confirm.</p>`;
    } else {
        document.getElementById('measurementInstruction').innerHTML = 
            `<p>Moving to ${currentPoint.label}. Position the nozzle at the center of the camera crosshair.</p>`;
    }
...
function toggleTestMode() {
    ...
    if (testMode.enabled) {
        ...
        document.getElementById('cameraText').textContent = 'Test Mode';
    } else {
        ...
        if (!camera) {
            document.getElementById('cameraStatus').textContent = '⚠️';
            document.getElementById('cameraText').textContent = 'Not started';
        }
    }
}
